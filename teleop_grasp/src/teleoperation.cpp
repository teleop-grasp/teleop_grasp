...
main(int argc, char** argv) 
{
	...
	ros::Subscriber sub_pose  = nh.subscribe<geometry_msgs::Pose>( topic_pose_hand, 1, [&](const auto& msg){ teleop_grasp::pose_hand = *msg;ROS_WARN_STREAM("hand pose cb: \n" << teleop_grasp::pose_hand.position); }); 
	...
	while (ros::ok()) 
	{
		...
		ROS_WARN_STREAM("hand pose received: \n" << teleop_grasp::pose_hand.position);
